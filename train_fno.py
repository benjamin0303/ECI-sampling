...
if __name__ == '__main__':
    ...
    if config.input_type == 'ic':
        def get_input(x):
            y = x.clone()
            y[..., 1:] = 0
            return y
    elif config.input_type == 'bc':
        def get_input(x):
            y = x.clone()
            y[:, 1:] = 0
            return y
    elif config.input_type == 'icbc':
        def get_input(x):
            y = x.clone()
            y[1:, 1:] = 0
            return y
    elif config.input_type == 'first15':
        def get_input(x):
            y = x.clone()
            y[..., 15:] = 0
            return y
    else:
        raise ValueError(f'Invalid condition type: {config.input_type}')
