...
if __name__ == '__main__':
    ...
    model = get_flow_model(config.model, config.encoder, conditional=True).to(args.device)
    ...
    if config.cond_type == 'ic':
        def get_cond(x):
            cond = x.clone()
            cond[..., 1:] = 0
            return cond
    elif config.cond_type == 'bc':
        def get_cond(x):
            cond = x.clone()
            cond[:, 1:] = 0
            return cond
    elif config.cond_type == 'round':
        def get_cond(x):
            cond = x.clone()
            cond[:, 1:-1, 1:-1] = 0
            return cond
    else:
        raise ValueError(f'Invalid condition type: {config.cond_type}')
    ...
    def sample_cond():
        model.eval()
        x = next(iter(test_loader)).to(args.device)[:config.n_sample]
        cond = get_cond(x)
        gen = model.sample(cond, config.n_eval)
